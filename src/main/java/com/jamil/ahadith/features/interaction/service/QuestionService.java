...
public class QuestionService {
    private static final Set<String> SCHOLAR_QUESTION_SORTS = Set.of("createdAt", "updatedAt", "id");
    ...
    public SearchResponse<ScholarQuestionResponseDto> getQuestions(int page, int size, String sort) {
        var pageable = adminPageService.pageable(page, size, sort, SCHOLAR_QUESTION_SORTS, DEFAULT_SCHOLAR_SORT);
        return adminPageService.response(questionRepository.findAllForScholar(pageable)
    ...
    public List<MemberQuestionResponseDto> getCurrentUserQuestions() {
        ...
        return questionRepository.findMemberQuestionsByAskerId(user.getId()).stream()
    ...
    public ScholarQuestionResponseDto getQuestionById(UUID id) {
        return questionRepository.findScholarQuestionById(id)
                .map(questionMapper::toScholarResponseDto)
    ...
    public MemberQuestionResponseDto getCurrentUserQuestionById(UUID id) {
        User user = currentUserService.requireCurrentUser();
        return questionRepository.findMemberQuestionByIdAndAskerId(id, user.getId())
                .map(questionMapper::toMemberResponseDto)
                .orElseThrow(QuestionNotFoundException::new);
    }

    public MemberQuestionResponseDto createQuestion(QuestionCreateRequestDto request) {
        ...
        return questionMapper.toMemberResponseDto(savedQuestion);
    }
    ...
    public void deleteCurrentUserQuestion(UUID id) {
        User user = currentUserService.requireCurrentUser();
        Question question = questionRepository.findMemberQuestionByIdAndAskerId(id, user.getId())
                .orElseThrow(QuestionNotFoundException::new);
        questionRepository.delete(question);
    }
