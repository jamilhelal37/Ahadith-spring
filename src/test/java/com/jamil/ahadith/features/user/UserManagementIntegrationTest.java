...
class UserManagementIntegrationTest {
    ...
    void adminShouldListSearchAndFilterUsersWithoutSensitiveFields() throws Exception {
        ...
        User activeMember =
                user(
                        "Alpha Search",
                        uniqueEmail("alpha-search"),
                        UserType.member,
                        UserStatus.active
                );

        User disabledScholar =
                user(
                        "Beta Scholar",
                        uniqueEmail("beta-search"),
                        UserType.scholar,
                        UserStatus.disabled
                );
        ...
        assertThat(
                firstItem.has("password")
        ).isFalse();

        assertThat(
                firstItem.has("tokenVersion")
        ).isFalse();
    ...
    void profileUpdateShouldRejectInvalidInputAndIgnoreProtectedFields() throws Exception {
        ...
        mockMvc.perform(
                        put("/api/v1/me")
                                ...
                                .content(
                                        "{\"name\":\"Valid Name\",\"gender\":\"unknown\",\"birthDate\":\"2000-01-01\"}"
                                )
                )
                .andExpect(status().isBadRequest());
        ...
        mockMvc.perform(
                        put("/api/v1/me")
                                ...
                                .content(
                                        "{\"name\":\"Valid Name\",\"gender\":\"male\",\"birthDate\":\"2999-01-01\"}"
                                )
                )
                .andExpect(status().isBadRequest());
        ...
        mockMvc.perform(
                        put("/api/v1/me")
                                ...
                                .content(
                                        "{\"name\":\"Valid Name\",\"gender\":\"male\",\"birthDate\":\"2020-01-01\"}"
                                )
                )
                .andExpect(status().isBadRequest());
    ...
    @Test
    void changePasswordShouldValidateRotateCredentialsInvalidateTokensWithoutAuditLog() throws Exception {
        User user =
                user(
                        "Password User",
                        uniqueEmail("password-user"),
                        UserType.member,
                        UserStatus.active
                );
    ...
    private void assertProfileUpdateWorks(
            User user
    ) throws Exception {
    ...
    private boolean activityLogExists(
            UUID recordId,
            String message
    ) {
